...
genai.configure(api_key='AIzaSyDJeqq-ttkymvsFWSys_cMDxB-8ivwGNdE')
...
response = short_model.generate_content('Write a 1000 word essay on importance of olives in modern society.')
print(response.text)
...
for _ in range(3):
    response = high_temp_model.generate_content('Pick a random colour ... (answer in a signle world)')
    if response.parts:
...
story_promp = 'You are a creative writeer. Write a short sotry about a cat who goes on an adcenture.'
response = model.generate_content(story_promp)
print(response.text)
